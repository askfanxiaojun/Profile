...
import { type BlogPostDetail } from '~/lib/mdx'
          ...
          <article data-post={post.slug}>
            <header className="relative flex flex-col items-center pb-5 after:absolute after:-bottom-1 after:block after:h-px after:w-full after:rounded after:bg-gradient-to-r after:from-zinc-400/20 after:via-zinc-200/10 after:to-transparent dark:after:from-zinc-600/20 dark:after:via-zinc-700/10">
              ...
              <motion.div
                ...
                transition={{ duration: 0.15, type: 'spring', stiffness: 150, damping: 20, delay: 0.1 }}
              >
                <time dateTime={post.date} className="flex items-center space-x-1.5">
                  <CalendarIcon />
                  <span>
                    {parseDateTime({ date: new Date(post.date) })?.format('YYYY/MM/DD')}
                  </span>
                </time>
              ...
              <motion.h1
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.2, type: 'spring', stiffness: 150, damping: 30, delay: 0.2 }}
              >
              ...
              <motion.p
                ...
                initial={{ opacity: 0, y: 8 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.2, type: 'spring', stiffness: 150, damping: 20, delay: 0.23 }}
              >
                {post.description}
              </motion.p>
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.15, type: 'spring', stiffness: 150, damping: 20, delay: 0.255 }}
              >
                <span className="inline-flex items-center space-x-1.5">
                  <HourglassIcon />
        ...
        <section className="mb-12 mt-32">
          ...
          <div className="mt-6 grid grid-cols-1 justify-center gap-6 md:grid-cols-[repeat(auto-fit,75%)] lg:grid-cols-[repeat(auto-fit,45%)] lg:gap-8">
            {relatedPosts.map((related) => (
              <BlogPostCard post={related} key={related.slug} />
            ))}
          </div>
        </section>
      )}
